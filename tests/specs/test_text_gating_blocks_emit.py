...
class TextGatingBlocksEmitTest(unittest.TestCase):
    def test_missing_required_marker_blocks(self):
        ...
        c = conn.cursor()
        rows = c.execute("select f.fid, e.path from findings f join evidence e on e.eid=f.primary_eid where replace(e.path,'\\','/') like '%text_signals/bad_missing_guard.py'").fetchall()
        try:
            conn.close()
        except:
            pass
