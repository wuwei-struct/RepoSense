import os
import json
import tempfile
import unittest
from reposense.scan import run_scan


class PythonAstFastApiRoutesTest(unittest.TestCase):
    def test_fastapi_routes_detected(self):
        repo = tempfile.mkdtemp(prefix="py_fastapi_repo_")
        p = os.path.join(repo, "main.py")
        with open(p, "w", encoding="utf-8") as f:
            f.write(
                "from fastapi import FastAPI\n"
                "app = FastAPI()\n"
                "BASE = \"/v1\"\n"
                "@app.get(BASE + \"/hello\")\n"
                "def hello():\n"
                "    return {\"ok\": True}\n"
                "@app.post(\"/pay\")\n"
                "def pay():\n"
                "    return {\"ok\": True}\n"
            )
        out = tempfile.mkdtemp(prefix="run_py_ast_")
        rd = run_scan(repo, out, os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "rulesets", "specs_v2")), os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "presets", "prod_lite.json")))
        rep = json.load(open(os.path.join(rd, "report.json"), "r", encoding="utf-8"))
        api = [x for x in rep.get("findings", []) if x.get("concept") == "API"]
        self.assertTrue(any("GET" in (a.get("snippet") or "") for a in api) or len(api) >= 1)


if __name__ == "__main__":
    unittest.main()

